      ...
      <div className="mx-auto max-w-6xl p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => void backfillDwg()} disabled={busy} className="rounded-lg border px-3 py-2 text-sm text-slate-600 hover:bg-slate-100 disabled:opacity-40" title="Importer les annotations des plans DWG comme commentaires">Importer du plan</button>
            <button onClick={() => void exportCsv()} className="rounded-lg border px-3 py-2 text-sm text-slate-600 hover:bg-slate-100">Export CSV</button>
        ...
        {loading ? (<p className="text-sm text-slate-500">Chargement…</p>) : view === 'list' ? (
        ...
        <div className="fixed inset-y-0 right-0 z-40 w-full max-w-md overflow-y-auto border-l bg-white shadow-xl">
          ...
          <div className="p-3">
            ...
            <div className="mt-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {selected.assignees.map((a) => (
                  <span key={a.id} className="inline-flex items-center gap-1 rounded bg-slate-100 px-1.5 py-0.5 text-[11px] text-slate-700">
            ...
            <div className="mt-3">
              ...
              <label className="mt-1.5 inline-block cursor-pointer rounded border border-dashed px-2 py-1 text-[11px] text-slate-500 hover:bg-slate-50">
                + Ajouter un fichier
                ...
                {selected.messages.map((m) => (
                  <div key={m.id} className="rounded bg-slate-50 p-2">
                    ...
                    <div className="mt-1 flex items-center gap-2 text-[10px] text-slate-400">
                      ...
                      <button onClick={() => { setEditMsgId(m.id); setEditMsgBody(m.body); }} className="hover:text-slate-700">éditer</button>
