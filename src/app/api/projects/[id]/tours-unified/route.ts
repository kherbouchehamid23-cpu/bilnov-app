...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [native, krpano, pano] = await Promise.all([
      ...
      prisma.file.findMany({ where: wf, select: { id: true, name: true, nodeId: true }, orderBy: { createdAt: 'desc' } }),
    ]);
    ...
    const items = [
      ...
      ...pano.map((f) => ({ id: f.id, name: f.name, status: 'PANO', nodeId: f.nodeId, kind: 'pano' })),
    ];
