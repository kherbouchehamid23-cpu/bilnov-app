...
export async function POST(req: NextRequest, { params }: { params: Record<string, string> }) {
  try {
    ...
    if (!fileId || !name) return apiError('fileId et name requis', 'VALIDATION_ERROR', 400);
    ...
    const scene = await prisma.tourScene.create({
      data: { tourId: params.tourId, name, position: count, isInitial: count === 0, imageUrl: storageKey },
    });
